...
import org.apache.flink.streaming.scala.examples.join.WindowJoin.{Grade, Salary,Money}
...
object WindowJoinSampleData {
  ...
  private[join] val MONEY_MAX = 10000
  ...
  def getMoneySource(env: StreamExecutionEnvironment, rate: Long): DataStream[Money] = {
    env.fromCollection(new ThrottledIterator(new MoneySource().asJava, rate).asScala)
  }
  ...
  class MoneySource extends Iterator[Money] with Serializable {
    ...
    def next: Money = {
      Money(NAMES(rnd.nextInt(NAMES.length)), rnd.nextInt(MONEY_MAX) + 1)
    }
  }
}
