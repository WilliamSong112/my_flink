...
object WindowJoin {
  ...
  case class Money(name: String, money: Int)
  
  case class Person(name: String, grade: Int, salary: Int)

  case class Person2(name: String, grade: Int, salary: Int,money:Int)
  ...
  def main(args: Array[String]) {
    ...
    val money = WindowJoinSampleData.getMoneySource(env, rate)

    // join the two input streams by name on a window.
    // for testability, this functionality is in a separate method.
    val joined = joinStreams(grades, salaries, windowSize).setParallelism(5)
    val joined4 = joinStreams2(joined,money,windowSize).setParallelism(5)
//    env.execute("Windowed Join Example")
    print(env.getStreamGraph.toString)
  }
  def joinStreams(
  ...
  def joinStreams2(
                    person: DataStream[Person],
                    money: DataStream[Money],
                   windowSize: Long) : DataStream[Person2] = {

    person.join(money)
      .where(_.name)
      .equalTo(_.name)
      .window(TumblingEventTimeWindows.of(Time.milliseconds(windowSize)))
      .apply { (p, m) => Person2(p.name, p.grade, p.salary,m.money) }
  }
}
