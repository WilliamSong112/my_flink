 ...
 * CEP模式检测的流抽象。模式流是一种被检测到的流
 模式序列作为与其名称相关联的事件的映射。模式是使用
 * {@link org.apache.flink.cep.nfa.NFA}。为了处理检测到的序列，用户
 *必须指定{@link PatternSelectFunction}或{@link PatternFlatSelectFunction}
 *
...
//	---------------
	 ...
	 * use {@link TimedOutPartialMatchHandler} as additional interface.
	 *
	 * 将过程函数应用于检测到的模式序列。对于每个模式，将
	 假设{@link PatternProcessFunction}被调用。为了尽可能地处理超时的部分匹配
	 使用{@link TimedOutPartialMatchHandler}作为附加接口。
	 *
	 * @param patternProcessFunction The pattern process function which is called for each detected
	 *                              pattern sequence.
	 * @param <R> Type of the resulting elements
