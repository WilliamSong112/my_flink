...
/**
  ...
  *
  * 这个[[RelNode]] DAG将分解为3个[[RelNodeBlock]]s，断点为[[RelNode]](' Join(a1=b2) ')，数据输出到多个[[Sink]]s。<p>提示:虽然‘Project(a,b,c)’有两个父结点(output)，
它们最终在‘Join(a1=b2)’处合并。所以‘Project(a,b,c)’不是断点。
<p>第一个[[RelNodeBlock]]包含表罐，项目(a,b,c)，过滤器(>0)，
Filter(c不为空)、Project(a1,b1)、Project(b2,c2)和Join(a1=b2)
<p>第二个包括Filter(a1>=70)， Project(a1,b1)和Sink(sink1)
<p>第三个包括Filter(a1<70)， Project(a1,c2)和Sink(sink2)
<p>和第一个[[RelNodeBlock]]是另外两个的子节点。
  */
...
object RelNodeBlockPlanBuilder {
  ...
  /**
    ...
    *
    * 将[[RelNode]]树分解为[[RelNodeBlock]]树。首先，将逻辑节点树转换为RelNode树。其次，在不同的树中重用相同的子计划。第三，将RelNode dag分解为[[RelNodeBlock]]树。
    *
    * resume 重用
